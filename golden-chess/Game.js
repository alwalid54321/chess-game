class Game {
	constructor(pieces) {
		...
		this.blackSematary = document.getElementById('blackSematary');
		   // تعريف اللاعبين ونقاطهم الافتراضية
	...
	changeTurn() {
		if (this.turn == 'white') {
			this.turn = 'black';
		this.turnSign.innerHTML = "golden's Turn";
		}
		else{
			this.turn = 'white';
		this.turnSign.innerHTML = "silver's Turn";
		}
	}
	...
	checkmate(color) {
		...
		const restartButton = endScene.querySelector('.restart-button');
	  
		winningSign.innerHTML = color + ' Wins';
		winningSign.style.color = color; // Change the color of the "Wins" text
		restartButton.style.display = 'block'; // Show the restart button
	  
		alert(color + ' Wins');
	  
		endScene.classList.add('show');
		endScene.classList.remove('hide');
		this.board.classList.add('hide');
		this.board.classList.remove('show');
		this.clearSquares();
	  
		let player1 = { name: "silver player", points: 0 };
		let player2 = { name: "golden player", points: 0 };
	  
		// Update player points based on the results
		function updatePoints(winner, loser) {
		  winner.points += 1; // Increase the winner's points by one
		  document.getElementById("player1Points").textContent = `${player1.points} point`;
		  document.getElementById("player2Points").textContent = `${player2.points} point`;
		}
	  
		// Use the function to update player points
